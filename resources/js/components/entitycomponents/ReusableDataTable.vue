...
const fetchRows = async (resetPagination = false) => {
    ...
    try {
        ...
        const response = await axios.post(`${props.baseentityurl}/list`, {
            ...
            sort_direction:
                sorting.value.length == 0 ? undefined : sorting.value[0]?.desc? 'desc' : 'asc',
        ...
        onPaginationChange: (updater) => {
            const newState =
                typeof updater === 'function'
                    ? updater(pagination.value.paginationState)
                    : updater;
    ...
    <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                        <DropdownMenuCheckboxItem
                            ...
                            @update:model-value="
                                (value: any) => column.toggleVisibility(!!value)
                            "
                        >
                            {{ column.id }}
                        </DropdownMenuCheckboxItem>
        ...
        <div class="flex items-center justify-end space-x-2 py-4">
            <div class="flex-1 text-sm text-muted-foreground">
                ...
                Total {{ pagination.total }} entities
            </div>
            <div class="flex items-center space-x-2">
                <p class="text-sm font-medium">Rows per page</p>
                <Select
                    :model-value="pagination.current_page.toString()"
                    @update:model-value="
                ...
                >
                    ...
                    <SelectContent side="top">
                        <SelectItem
                            v-for="size in pageSizesOptions"
                            :key="size"
                            :value="size.toString()"
                        >
                            {{ size }}
